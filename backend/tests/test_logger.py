...
def test_setup_logging_creates_file_handler(tmp_path):
    ...
    root = logging.getLogger("murmura_test")
    root.handlers.clear()
    with patch.dict(os.environ, {"LOG_FILE": str(log_path), "LOG_LEVEL": "DEBUG"}):
        result = lg.setup_logging(logger_name="murmura_test")
...
def test_setup_logging_default_level_info():
    ...
    root = logging.getLogger("murmura_default")
    root.handlers.clear()
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("LOG_LEVEL", None)
        result = lg.setup_logging(logger_name="murmura_default")
